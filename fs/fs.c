...
{
	u_int offset = blockno * BY2BLK;
	if (super && blockno > super->s_nblocks) {
		user_panic("diskaddr error: DIskno too large!\n");
	}
...
unmap_block(u_int blockno)
{
	...
	if (syscall_mem_unmap(0, addr) < 0) {
		user_panic("unmap_block: could not unmap page");
	}
...
// 	On error return < 0.
int
self_file_create(char *path, struct File **file)
{
	char name[MAXNAMELEN];
	int r;
	struct File *dir, *f;
