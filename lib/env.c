                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    for (i = 0; i < bin_size + offset; i += BY2PG) {
		...
		right = MIN(BY2PG, bin_size + offset - i);
		bcopy(bin + i - offset + left, (void *)page2kva(p) + left, right - left);
    }
...
env_create_priority(u_char *binary, int size, int priority)
{
	...
	if ((r = env_alloc(&e, 0)) < 0) {
		panic("env_create_priority allocate env fail at: %d\n", r);
	}
