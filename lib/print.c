  ...
  { if (((l) < 0) || ((l) > LP_MAX_BUF)) { \
    ...
    */
	...
	{ 
		...
		OUTPUT(arg, fmt, probe - fmt);
		fmt = probe;	//if find a %, now fmt = %
	...
	/* we found a '%' */
	fmt++;
	/* check for long */		//So weird...should be 'check format flag'??
	ladjust = 0;
	padc = ' ';
	...
	if (*fmt == '.') {
		...
		prec = 0;
		while (IsDigit(*fmt)) {
			prec = Ctod(*fmt) + prec * 10;
			fmt++;
		}
	}	

	/* check format flag */		//Weird too. should be 'check for length'??
	longFlag = 0;
	if (*fmt == 'l') {
		longFlag = 1;
		fmt++;
