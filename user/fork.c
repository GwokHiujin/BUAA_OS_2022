...
pgfault(u_int va)
{
	...
	if (!(perm & PTE_COW)) {
		user_panic("pgfault: This page is not a Copy_On_Write page.\n");
	}
	perm = (perm & ~PTE_COW) | PTE_R;
	//map the new page at a temporary place
	tmp = (void *)USTACKTOP;
	if (syscall_mem_alloc(0, (u_int)tmp, perm) < 0) {
		user_panic("pgfault: New page cannot be allocated.\n");
	}
	//copy the content
	user_bcopy((void *)va, tmp, BY2PG);
	//map the page on the appropriate place
	if (syscall_mem_map(0, (u_int)tmp, 0, va, perm) < 0) {
		user_panic("pgfault: Cannot map the page.\n");
	}
	//unmap the temporary place
	if (syscall_mem_unmap(0, (u_int)tmp) < 0) {
		user_panic("pgfault: Cannot unmap the temporary place.\n");
	}
}
...
fork(void)
{
	...
	if (syscall_mem_alloc(newenvid, UXSTACKTOP - BY2PG, PTE_V | PTE_R) < 0) {
		user_panic("fork: Cannot allocate stack.\n");
	}
    if (syscall_set_pgfault_handler(newenvid, __asm_pgfault_handler, UXSTACKTOP) < 0) {
        user_panic("fork: Cannot set page fault handler.\n ");
    }
    if (syscall_set_env_status(newenvid, ENV_RUNNABLE) < 0) {
        user_panic("fork: Cannot set child process's status.\n");
    }
