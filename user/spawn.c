...
usr_load_elf(int fd , Elf32_Phdr *ph, int child_envid){
	...
	for (i = ROUNDDOWN(ph->p_vaddr, BY2PG); i < ph->p_vaddr + ph->p_memsz; i += BY2PG) {
		...
		right = MIN(i + BY2PG, ph->p_vaddr + ph->p_filesz);
		if (right > i) {
			//left = MAX(i, ph->p_vaddr);
			left = (i > ph->p_vaddr) ? i : ph->p_vaddr;
			if ((r = readn(fd, (char *)(left - i + TMPPAGE), right - left)) < 0) {
				return r;	
			}	
		}
...
int spawn(char *prog, char **argv)
{
	...
	int fd;
	u_int child_envid;
	int entry_size, text_start, count;
	u_int i, *blk;
	u_int esp;
	Elf32_Ehdr *ehdr;
	Elf32_Phdr *phdr;
	...
	*/
	...
	if ((r = readn(fd, elfbuf, sizeof(Elf32_Ehdr))) < 0) {
		user_panic("spawn failed at reading Ehdr");	
	}
	ehdr = (Elf32_Ehdr *)elfbuf;
	if ((!usr_is_elf_format((u_char*)ehdr)) || ehdr->e_type != 2) {
		user_panic("spawn failed at finding elf or exec");	
	}
	...
	if (child_envid < 0) {
		user_panic("spawn failed at allocating env");	
	}
